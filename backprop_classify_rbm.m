...
w1=[vishid; hidrecbiases]; %% v*(h+1) matrix
...
%  targetout = exp(w3probs*w_class);
  targetout = exp(w1probs*w_class); % directly after w1
...
%  w3probs = 1./(1 + exp(-w2probs*w3)); w3probs = [w3probs  ones(N,1)];
%  targetout = exp(w3probs*w_class);
  targetout = exp(w1probs*w_class); % direct after w1
  targetout = targetout./repmat(sum(targetout,2),1,10);
...
%    w3probs = 1./(1 + exp(-w2probs*w3)); %w3probs = [w3probs  ones(N,1)];
    ...
    VV = [w_class(:)']';
    Dim = [l4; l5];
    [X, fX] = minimize(VV,'CG_CLASSIFY_INIT_RBM',max_iter,Dim,w1probs,targets);
    ...
    % Dim = [l1; l2; l3; l4; l5];
    Dim = [l1; l4; l5];
    [X, fX] = minimize(VV,'CG_CLASSIFY_RBM',max_iter,Dim,data,targets);

    w1 = reshape(X(1:(l1+1)*l4),l1+1,l4);
    xxx = (l1+1)*l4;
